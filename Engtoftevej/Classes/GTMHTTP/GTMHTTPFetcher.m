...
- (void)handleCookiesForResponse:(NSURLResponse *)response {
  ...
  else if ([(NSHTTPURLResponse *)response respondsToSelector:@selector(allHeaderFields)]) {
...
-(void)connection:(NSURLConnection *)connection
	...
	NSArray *trustedHosts = [NSArray arrayWithObject:@"et1.mitnykredit.dk"];

	if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
		//NSLog(@"ss=%@",challenge.protectionSpace.host);
...
- (void)setCookieStorageMethod:(NSInteger)method {
  ...
  {
      // Don't modify HTTPShouldHandleCookies. Default is YES.
    // system default
