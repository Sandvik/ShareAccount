...
- (NSMutableArray *)removeDuplicateObjects
{
    ...
    for (NSDictionary *d in selfTemp)
    {
        ...
        for(int i=0;i<[removedTemp count];i++){
            NSDictionary *dict =[removedTemp objectAtIndex:i];
            //        }
            ...
            if ([str1 isEqualToString:str2]){
                ...
                if (count > 1){
                    ...
                    //                    break;
                    [removedTemp removeObjectAtIndex:i];
                    //index = [removed indexOfObject:[removed lastObject]];
                    //removedTemp = nil;
                    //removedTemp = [removed reverse];
                    break;
...
{
	[super viewDidLoad];
    upperBar.topItem.title = @"Choose Contacts";
	...
	for (int i = 0; i < nPeople; i++)
	{
        ...
        if ((objs != @"") || ([[objs lowercaseString] rangeOfString:@"null"].location == NSNotFound))
		{
            ...
            {
                NSLog(@" EMail = %@",objs);
                if ([objs isEqualToString:@""] || objs ==nil) {
                    //NSLog(@" EMail = %@",objs);
                    info =nil;
                } else {
                    //NSLog(@" EMail = %@",objs);
   ...
   // NSLog(@"%@",temp);
    ...
    //Fjerner alle der allerede er inkluderet i regnskab
    for(int i=0;i<[temp count];i++){
        NSDictionary *dict =[temp objectAtIndex:i];              
        NSString *emailStr = [NSString stringWithFormat:@"%@", [dict valueForKey:@"email"]];
        if ([self checkForAlleredeInviteret:emailStr]) {
...
-(BOOL)checkForAlleredeInviteret:(NSString*)streng{
    NSArray *chunks = [streng componentsSeparatedByString: @","];
    NSArray *peopleArray = [AppDataCache sharedAppDataSource].peopleList;
    for(int i=0;i<[chunks count];i++){
        NSString *email =[chunks objectAtIndex:i];
        for(int j=0;j<[peopleArray count];j++){
            NYKGeneralAccount *personname =[peopleArray objectAtIndex:j];
            if( [email caseInsensitiveCompare:personname.email] == NSOrderedSame ) {
                return YES;
            }
        }
    }
